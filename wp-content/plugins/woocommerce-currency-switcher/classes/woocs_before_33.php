...
//*** checking and assigning data
    ...
    public function plugin_action_links($links) {
        return array_merge(array(
            ...
            '<a target="_blank" href="' . esc_url('http://currency-switcher.com/documentation/') . '">' . esc_html__('Documentation', 'woocommerce-currency-switcher') . '</a>'
                ), $links);
    ...
    public function widgets_init() {
        ...
        wp_register_script('wc-price-slider', WOOCS_LINK . 'js/price-slider.js', array('jquery', 'jquery-ui-slider', 'wc-jquery-ui-touchpunch'), WOOCS_VERSION);
    }
...
//***
        ...
        wp_add_inline_script('wc-price-slider', $this->init_js_properties(), 'before');
...
//[woocs_show_custom_price value=20] -> value should be in default currency
    ...
    public function woocs_geo_hello($atts = '') {
        ...
        if (!empty($pd) AND ! empty($countries)) {
            $text = '<span style="color:green; font-size:18px; font-weight:normal; line-height:0;">' . sprintf(esc_html__('Your country is: <b>%s</b>. <br /><i style="color:black; font-size:11px; font-weight:normal;">(defined by woocommerce GeoIP functionality)</i>', 'woocommerce-currency-switcher'), $countries[$pd['country']]) . '</span>';
        } else {
...
//as it recounted twice - down it!
    ...
    function woocs_alert() {
        ...
        if (intval(get_option('woocs_alert_rev')) + 86400 * 7 <= time()) {
            if (isset($_GET['tab']) AND $_GET['tab'] == 'woocs') {
                ...
                <div class="notice notice-warning is-dismissible" id="woocs_alert_notice" data-nonce="<?php echo json_encode(wp_create_nonce('woocs_dissmiss_alert')) ?>">
                    <p class="plugin-card-woocommerce-currency-switcher">
                        <?php printf(__("Hello! Looks like you using <b>WooCommerce Currency Switcher</b> for some time and I hope this software helped you with your business. If you happy with the plugin functionality rate please WOOCS with 5-stars review, also share your opinion and ideas with us. Thank you!<br /> P.S. If you have troubles you can always ask %s about help.", 'woocommerce-currency-switcher'), "<a href='{$support_link}' target='_blank'>" . __('support', 'woocommerce-currency-switcher') . "</a>") ?> 
                    </p>
